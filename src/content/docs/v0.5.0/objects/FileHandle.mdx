...
read from the file
```luau tooltips
:read(amount: number?, bytes: nil): string
...
- `bytes: nil | false | true | boolean{:luau}` - If true, the function will return a `buffer` instead of a `string`.
</details>
<details>
...
```luau tooltips
:readSync(amount: number?, bytes: nil): string
